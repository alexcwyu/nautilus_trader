...
//! Integration tests for the Architect HTTP client using a mock Axum server.

use std::{collections::HashMap, net::SocketAddr, path::PathBuf, time::Duration};

use axum::{Router, http::StatusCode, response::Json, routing::get};
use nautilus_architect::http::{
    client::{ArchitectHttpClient, ArchitectRawHttpClient},
...
async fn test_raw_http_get_instruments_returns_data() {
    ...
    let client =
        ArchitectRawHttpClient::new(Some(base_url), None, Some(60), None, None, None, None)
            .unwrap();
...
async fn test_raw_http_get_balances_returns_data() {
    ...
    let mut client = ArchitectRawHttpClient::with_credentials(
...
async fn test_domain_http_request_instruments_returns_nautilus_types() {
    ...
    let client =
        ArchitectHttpClient::new(Some(base_url), None, Some(60), None, None, None, None).unwrap();
...
async fn test_domain_http_request_instrument_returns_nautilus_type() {
    ...
    match instrument {
        InstrumentAny::CryptoPerpetual(perp) => {
            ...
            assert_eq!(perp.id.venue.as_str(), "ARCHITECT");
        }
...
async fn test_http_network_error_invalid_port() {
    ...
    let client =
        ArchitectRawHttpClient::new(Some(base_url), None, Some(1), Some(0), None, None, None)
            .unwrap();
    ...
    match result {
        Err(ArchitectHttpError::NetworkError(_)) => {}
...
async fn test_http_500_internal_server_error() {
    ...
    let base_url = format!("http://{addr}");
    let client =
        ArchitectRawHttpClient::new(Some(base_url), None, Some(60), Some(0), None, None, None)
            .unwrap();
    ...
    assert!(result.is_err());
    match result {
        Err(ArchitectHttpError::UnexpectedStatus { status, .. }) => {
...
async fn test_http_malformed_json_response() {
    ...
    assert!(result.is_err());
    match result {
        Err(ArchitectHttpError::JsonError(_)) => {}
