...
async fn test_rapid_consecutive_reconnections() {
    ...
    for i in 0..3 {
        ...
        // Trigger reconnection by sending a subscription
    ...
    // Connection count may have increased due to reconnections
...
async fn test_reconnection_race_condition() {
    ...
    // Trigger disconnect during active operation
    state.disconnect_trigger.store(true, Ordering::Relaxed);
    let _ = client
    ...
    // Trigger another disconnect while potentially reconnecting
    state.disconnect_trigger.store(false, Ordering::Relaxed);
...
async fn test_reconnection_waits_for_delayed_auth_ack() {
    ...
    // Set auth delay to simulate slow auth response
    ...
    // Connect with delayed auth
    let _ = client.connect().await;

    // Wait for connection with delayed auth
    wait_until_async(
        || async { *state.connection_count.lock().await > 0 },
        Duration::from_secs(3),
    )
    ...
    // Give time for auth delay and subscription attempt
    tokio::time::sleep(Duration::from_millis(1000)).await;

    // Connection should be maintained despite auth delay
    assert!(
...
async fn test_multiple_partial_subscription_failures() {
    ...
    // Subscribe to multiple topics successfully
    let topics = vec![
        "publicTrade.BTCUSDT".to_string(),
        "publicTrade.ETHUSDT".to_string(),
        "orderbook.50.BTCUSDT".to_string(),
    ...
    // Set one subscription to fail on next attempt
    state
    ...
    // Try to subscribe to a mix of topics (one will fail)
...
async fn test_is_active_false_during_reconnection() {
    ...
    if !active_during_reconnect {
        // Expected behavior - client was inactive during reconnection
...
async fn test_sends_pong_for_text_ping_message() {
    ...
    // Verify that pings were sent and responded to
    assert!(
