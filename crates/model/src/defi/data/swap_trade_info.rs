...
pub struct SwapTradeInfoCalculator<'a> {
    ...
    token0: &'a SharedToken,
    /// Reference to token1 from the pool.
    token1: &'a SharedToken,
    /// Whether the base/quote assignment differs from token0/token1 ordering.
    ///
...
mod tests {
    ...
    #[rstest]
    fn test_swap_trade_info_calculator_calculations_buy(weth: SharedToken, usdc: SharedToken) {
    ...
    #[rstest]
    fn test_swap_trade_info_calculator_calculations_sell(weth: SharedToken, usdc: SharedToken) {
