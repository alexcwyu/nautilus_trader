...
class TestImmediateQuoteExecution:
    ...
    def test_same_timestamp_fill_without_message_queue(self):
        """
        Order fills on the same quote tick it was submitted.
        """
        quotes = self._make_quotes()
        strategy = self._run_with_message_queue(quotes, use_message_queue=False)
    ...
    def test_both_queue_modes_fill_on_same_tick(self):
        """
        Both use_message_queue modes now fill on the first quote tick.
        """
        quotes = self._make_quotes()
        strategy_no_queue = self._run_with_message_queue(quotes, use_message_queue=False)
        strategy_with_queue = self._run_with_message_queue(quotes, use_message_queue=True)

        assert strategy_no_queue.fill_timestamps[0] == quotes[0].ts_init
        assert strategy_with_queue.fill_timestamps[0] == quotes[0].ts_init
        assert strategy_no_queue.fill_timestamps[0] == strategy_with_queue.fill_timestamps[0]
...
class BarLimitOrderStrategy(Strategy):
    ...
    The limit price is configurable so we can test both cases:
      - price above close -> fills immediately via is_limit_matched
...
class TestBarSameTimestampExecution:
    ...
    is_limit_matched check (against close state).
    ...
    def test_limit_buy_above_close_fills_on_same_bar(self):
        ...
        is_limit_matched in process_order sees price >= ask, so the order fills on the
        same bar.
