...
def test_position_notional_value_conversion():
    instrument = TestInstrumentProvider.btcusdt_binance()
    fill = create_fill(instrument, OrderSide.BUY, Quantity.from_str("1.0"), Price.from_str("50000.0"))
...
def test_portfolio_query_methods_with_conversion():
    ...
    # Create a position
    fill = create_fill(btcusdt, OrderSide.BUY, Quantity.from_str("1.0"), Price.from_str("50000.0"), account_id=account_id)
...
def test_portfolio_realized_pnl_with_conversion():
    ...
    fill1 = create_fill(btcusdt, OrderSide.BUY, Quantity.from_str("1.0"), Price.from_str("50000.0"), account_id=account_id)
    pos = Position(instrument=btcusdt, fill=fill1)
    cache.add_position(pos, OmsType.NETTING)

    # Close position at higher price
    fill2 = create_fill(btcusdt, OrderSide.SELL, Quantity.from_str("1.0"), Price.from_str("55000.0"), account_id=account_id)
    pos.apply(fill2)
...
def test_portfolio_total_pnl_with_currency_mismatch():
    ...
    fill1 = create_fill(btcusdt, OrderSide.BUY, Quantity.from_str("2.0"), Price.from_str("50000.0"), account_id=account_id)
    pos = Position(instrument=btcusdt, fill=fill1)
    cache.add_position(pos, OmsType.NETTING)
...
def test_portfolio_net_exposure_multiple_positions():
    ...
    fill2 = create_fill(btcusdt, OrderSide.BUY, Quantity.from_str("0.5"), Price.from_str("51000.0"), account_id=account_id)
...
def test_position_cross_notional_value_inverse_pair():
    ...
    fill = create_fill(usdjpy, OrderSide.BUY, Quantity.from_str("100000.0"), Price.from_str("150.0"))
...
def test_portfolio_net_exposure_currency_pair_with_cross_notional():
    ...
    # Create position
    fill = create_fill(audusd, OrderSide.BUY, Quantity.from_str("100000.0"), Price.from_str("0.75"), account_id=account_id)
    ...
    cache.add_quote_tick(TestDataStubs.quote_tick(instrument=audeur, bid_price=0.675, ask_price=0.675))

    # Update AUD/USD price
    cache.add_quote_tick(TestDataStubs.quote_tick(instrument=audusd, bid_price=0.75, ask_price=0.75))
...
def test_position_cross_notional_value_multiple_scenarios():
    ...
    # Test EUR/USD to GBP
    eurusd = TestInstrumentProvider.default_fx_ccy("EUR/USD", Venue("SIM"))
    fill1 = create_fill(eurusd, OrderSide.BUY, Quantity.from_str("100000.0"), Price.from_str("1.10"))
